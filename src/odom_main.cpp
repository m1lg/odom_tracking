...
public:
    OdometryNode( ros::NodeHandle ext_nh ) :
        po_nh( ext_nh )
    {   
        odom_pub = po_nh.advertise<nav_msgs::Odometry>("odom", 50);
    }
    ...
    void cb_joint(const sensor_msgs::JointState::ConstPtr& msg) {// Define a function called 'callback'
                                                                             // that receives a parameter named 'msg'                                                                   
        std::vector<std::string> names = msg->name;	
        if (msg->name[0][0] == 'f') {                              //if the first letter of the first name in the
                                                                   // array is f then we have the velocity data
            double angular_v;
            angular_v = (msg->velocity[1]+msg->velocity[2])/2;     //centre of vehicle/baselink velocity wheel be the
                                                                   // average of the two rear wheel velocities
            ...
            sensor_data.dt = sensor_data.time2 - sensor_data.time1;  //dt is the difference between current time and
                                                                     // previous time 
            
            if ((sensor_data.dt.toSec() < 1)&&(sensor_data.dt.toSec() > 0)){   //first time is un initialised and >
                                                                               //than 1, this ensures we disregard that
                calculateOdom();
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber sub_joint_states = nh.subscribe("/zio/joint_states", 1000, &OdometryNode::cb_joint, &t);   
    ros::Subscriber sub_imu = nh.subscribe("/vn100/imu", 1000, &OdometryNode::cb_IMU, &t); 
    ros::Subscriber sub_gps = nh.subscribe("/ublox_gps/fix", 1000, &OdometryNode::cb_GPS, &t); 
